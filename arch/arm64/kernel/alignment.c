...
static int align_ldst_unpriv(u32 insn, struct pt_regs *regs)
{
	const u32 SIZE = GENMASK(31, 30);
	const u32 OPC = GENMASK(23, 22);
	...
	if (!is_ttbr0_addr(address))
		return 1;

	if (is_store) {
		data = pt_regs_read_reg(regs, t);
		if (align_store(address, datasize / 8, data))
			return is_ttbr1_addr(regs->pc) &&
				fixup_exception(regs, ESR_ELx_WNR) ? 0 : 1;
	} else {
		if (align_load(address, datasize / 8, &data))
			return is_ttbr1_addr(regs->pc) &&
				fixup_exception(regs, 0) ? 0 : 1;
		if (is_signed) {
			if (regsize == 32)
				data = sign_extend32(data, datasize - 1);
			else
...
static int align_ldst(u32 insn, struct pt_regs *regs)
{
	...
	if ((op0 & 0x3) == 0x2) {
		 ...
		 * |------+-----+-----+--------+-----+------------------------------------|
		 */
		return align_ldst_unpriv(insn, regs);
	} else
		return 1;
}
